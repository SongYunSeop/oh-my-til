...
Add `sources` field to TIL frontmatter:
...
date: 2026-04-13
category: react
tags: [til, react, hooks]
sources:
  - raw/react/react-hooks-guide.md
  - https://react.dev/reference/hooks
---
```

Sources is a flat list of raw file paths and/or URLs. Optional field — TILs created without raw sources (e.g., from direct learning) omit it.

### /til-lint

Two new checks:

1. **Unprocessed raw**: Files in `raw/` not referenced by any TIL's `sources` field
2. **Broken raw reference**: TIL `sources` contains a raw path but the file doesn't exist

## Tracking: TIL references raw (one-directional)

- TIL frontmatter `sources` field points to raw files
- Raw files are never modified (immutable)
- Reverse lookup (raw -> TIL) is computed dynamically by scanning all TIL sources fields (lint, dashboard, etc.)
